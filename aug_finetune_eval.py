...
try:
    ...
    warnings.warn(
        ...
        "stripped. Ensure step1_analysis.py is importable (same directory "
        "or on PYTHONPATH)."
    )
...
def _train_pipeline(pipeline_id, aug_df, dev_df, adapter_out_dir,
    ...
    # Count after dropping nulls — same logic as _build_dataset uses
...
def _eval_pipeline(pipeline_id, test_df, adapter_out_dir,
    ...
    sub = sub.dropna(subset=["src_text","tgt_text"]).reset_index(drop=True)
    if len(sub) == 0:
    ...
    if not _has_final_adapter(adapter_out_dir):
        raise FileNotFoundError(
            f"No final adapter at '{adapter_out_dir}'. Training not complete."
        )
...
def run_finetune_and_eval(
    score_key:        str,
    dataset_files:    dict,          # {pipeline_id: path}
    adapter_out_dirs: dict,          # {pipeline_id: path}
    pred_files:       dict,          # {pipeline_id: path}
    pipelines:        list = None,
    start_adapters:   dict = None,   # {pipeline_id: path}  defaults to R1
    label:            str  = "",
    use_subword_reg:  bool = True,
) -> dict:
    """
    Fine-tune and evaluate for one or more pipelines independently.

    Args:
        score_key       : base key for all_scores.json (e.g. "exp1")
        dataset_files   : {pipeline_id: aug_dataset_xlsx_path}
        adapter_out_dirs: {pipeline_id: output_adapter_path}
        pred_files      : {pipeline_id: prediction_csv_path}
        pipelines       : list of pipeline IDs to process; defaults to ALL_PIPELINES
        start_adapters  : {pipeline_id: starting_adapter_path}; defaults to R1
        label           : human-readable label for logs
        use_subword_reg : enable subword regularisation at training time

    Returns:
        Dict of {pipeline_id: metrics_dict}
    """
    ...
    # ── Fine-tune each pipeline ────────────────────────────────────────────────
    for pid in pipelines:
        aug_df = pd.read_excel(dataset_files[pid])
